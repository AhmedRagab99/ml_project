...
def KNN_Classifier():
    ...
    keys = list(y_test.keys())
    newytest = []

    for i in range(len(Prediction)):
        newytest.append(y_test[keys[i]])

    print('KNN Accuracy:')
    Accuracy = sklearn.metrics.accuracy_score(newytest, Prediction)
    print(Accuracy)
    print('Error:')
...
def Polynomial_KernalSVM():
    ...
    return [model1.score(X_test, y_test), trainig_time, test_time]
...
drawBarGraph('Accuracy', 'Value', ['KNN Accuracy', 'Decision tree Accuracy', 'AdaBoost  Accuracy'],
             [KNN[0], Decisiontree[0], AdaBoost[0]], 'Accuracy Graph')
drawBarGraph('Training Time', 'Value', ['KNN Training time', 'Decision tree Training time', 'AdaBoost Training time'],
             [KNN[1], Decisiontree[1], AdaBoost[1]], 'Training time Graph')
drawBarGraph('Test time', 'Value', ['KNN Test time', 'Decision tree Test time', 'AdaBoost  Test time'],
             [KNN[2], Decisiontree[2], AdaBoost[2]], 'Test time Graph')
